...
eachSeries(links, (link, callback) => {
  ...
  .then(html => {
    ...
    const opt = {
      portrait: ['fsc', 'fs11', 'ff1'],
      landscape: ['fsf', 'fs11', 'ff1']
    };
    ...
    download(url).pipe(fs.createWriteStream(`${file}.pdf`).on('finish', () => {
      ...
      converter.convert('ipad').then(() => {
        ...
        let $ = cheerio.load(fs.readFileSync(`${file}.html`));
        if ($('.pc.h24').length > 0) {
          classes = opt.landscape;
        } else {
          classes = opt.portrait;
        }
        $ = null;
          ...
          $$(elem).find('.pc > *').each((i, element) => {
            if (element.name === 'div') {
              if ((element.attribs.class.indexOf(classes[0]) > -1) ||
                ...
                if (((element.next.attribs.class.indexOf(classes[0]) > -1) &&
                     (isNaN(element.next.children[0].data[0]))) ||
                    ((element.next.attribs.class.indexOf(classes[1]) > -1) &&
                     (isNaN(element.next.children[0].data[0])))
                   ) {
                  if (header === '') {
                    header += $$(element).text();
