  ...
  server.post('/webhook', (req, res) => {
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      if (event.postback) {
        ...
        Docs.findById(event.postback.payload, (err, item) => {
          if (err) {
            console.error(err);
          } else {
            if (item.length == 0) {
            ...
            let id = item.docid;
            let textToSend = item.content;
            let i = 0;
            let substring = textToSend.substring(i,i+300);
            let isLastMessage = false;

            sendRequest(event.sender.id, event, substring, isLastMessage, textToSend, i, id);
